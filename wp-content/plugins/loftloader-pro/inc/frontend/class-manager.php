	...
	class LoftLoader_Pro_Frontend_Manager {
		...
		*/
		public function __construct() {
			...
			add_filter( 'loftloader_pro_loader_enabled', array( $this, 'detect_page_builder' ), 9999 );
		}
		/**
		...
		public function is_loader_enabled() {
			if ( ( 'on' === esc_attr( llp_get_loader_setting( 'loftloader_pro_main_switch') ) ) && $this->device_test() ) {
				...
				switch ( $range ) {
					...
					case 'homepage-once':
						if ( is_front_page() && $this->once_per_session_test( 'homepage') ) {
			    			$_SESSION['loftloader-pro-homepage-visited'] = true;
							return true;
						}
						break;
					case 'once':
						if ( $this->once_per_session_test() ) {
		...
		*/
		protected function check_cookies() {
			...
			if ( in_array( $range, $onces ) && ! is_customize_preview() && ! headers_sent() ) {
				...
				llp_set_cookie( 'wordpress-loftloader_pro_session', 'on' );
				nocache_headers();
			}
		}
		/**
		...
		*/
		protected function once_per_session_test( $id = '' ) {
			if ( $this->enabled_session ) {
				...
				return empty( $_COOKIE[ $cookie_id ] ) || ( 'on' !== sanitize_text_field( wp_unslash( $_COOKIE[ $cookie_id] ) ) );
			} else {
				return true;
			}
		}
