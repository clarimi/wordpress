...
( function( $ ) {
	...
	var $DOMBody = $( 'body' ), leavingShowAll = loftloaderPro['insiteTransitionShowAll'] === 'undefined' ? false : loftloaderPro['insiteTransitionShowAll'],
		...
		minTimer = '', maxTimer = '', rapidPercentage = parseInt( ( leavingProgressMax * 90 ), 10 );
	...
	function llp_check_random_message() {
		...
		if ( $message.length && message ) {
			...
			window.sessionStorage.setItem( 'loftloader-pro-next-random-message', message );
		}
	}
			...
			if ( href && llp_check_a( $item ) ) {
				$item.off( 'click' ).on( 'click', function( e ) {
					...
					if ( leavingShowAll ) {
						...
						window.sessionStorage.setItem( 'loftloader-pro-smooth-transition', 'on' );
					}
					setTimeout( function() {
	...
	function llp_loader_type_load_bg_span( $span, $image ) {
		if ( $span.length && $image.length ) {
			...
			$span.css( { 'width': ( values.width || '100%' ), 'height': ( values.height || '100%' ), 'display': '' } );
		}
	}
	...
	function llp_show_random_message() {
		var $message = $( '#loftloader-wrapper .loader-message' ), message = llp_get_random_message(),
			presetMessage = window.sessionStorage.getItem( 'loftloader-pro-next-random-message' );
		if ( $message.length && message ) {
			presetMessage ? $message.html( presetMessage ) : $message.html( message );
		}
	}
	// Get random message
